...
O modelo é servido localmente via Ollama (https://ollama.com) — gratuito,
roda no próprio computador e não exige nenhuma chave de API."""
...
MODEL = os.environ.get("OTINIEL_MODEL", "llama3.2")
...
def gerar_resposta(pergunta: str, historico: list, client: "ollama.Client") -> str:
    """Gera a resposta do Otiniel usando a base de conhecimento + histórico da conversa.

    `client` é um ollama.Client apontando para o servidor local do Ollama
    ...
    except Exception as exc:  # ConnectionError do Ollama fora do ar, modelo não baixado, etc.
        return (
            "⚠️ Não consegui falar com o Ollama. Confirme que:\n\n"
            "1. O Ollama está instalado e rodando (`ollama serve`, ou o app do Ollama aberto);\n"
            f"2. O modelo `{MODEL}` foi baixado (`ollama pull {MODEL}`).\n\n"
